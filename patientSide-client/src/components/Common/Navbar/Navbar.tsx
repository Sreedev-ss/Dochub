...
const Navbar = () => {
  ...
  const handleInputChange = async(event:any) => {
    ...
    const {data}:any = await fetchSuggestions(value);
    setSuggestions(data)
  };
  

  const fetchSuggestions = async(value:string) => {
        return makeApiCall(`/doctor/search-doctor?query=${value}`,'GET')
  };
      ...
      <Close onClick={()=>setShowSearch(false)} sx={{color:'white'}} className='mt-2'/>
        <div className="w-4/6 z-50 relative mx-auto mt-36 overflow-scroll" >
          <div className="bg-white w-full h-16 rounded-xl mb-3 shadow-lg p-2">
            <input onChange={handleInputChange} type="text" placeholder="Search" value={inputValue} className="w-full h-full text-2xl rounded-lg focus:outline-none focus:ring focus:border-blue-300" />
          </div>
